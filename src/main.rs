		...
		for h in 0..(CAM_H / TILE_SIZE) + 1 {
			for w in 0..(CAM_W / TILE_SIZE) + 1 {
				...
				if h as i32 + h_bounds_offset < 0 ||
					...
					match num {
						1 => { // floor tiles
							src.x = 8;//dirt_map[h as usize][w as usize].0*64 + dirt_map[h as usize][w as usize].0*8; 
							src.y = 8;//dirt_map[h as usize][w as usize].1*64 + dirt_map[h as usize][w as usize].1*8; 
