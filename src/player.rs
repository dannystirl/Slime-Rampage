...
impl<'a> Player<'a> {
	...
	// update player
	pub fn update_player(&mut self, game_data: &GameData, mut map: [[i32; MAP_SIZE_W]; MAP_SIZE_H]) {
		...
		/*for h in 0..(CAM_H / TILE_SIZE) + 1 {
			for w in 0..(CAM_W / TILE_SIZE) + 1 {
				let p_pos = self.pos();
				if map[h as usize][w as usize] == 2 { // 2 is a wall 
					...
					TILE_SIZE, TILE_SIZE);
					if GameData::check_collision(&self.pos(), &w_pos) {//I hate collisions
